...
**Toast**, new here, is a five-variant (success/warning/info/danger/neutral) inline notification — a solid left accent bar plus a soft-tinted background, both composed entirely from this system's existing semantic tokens rather than a new toast-specific palette: success uses `border-success`/`success-soft`, warning uses `content-warning`/`warning-soft` (no dedicated `border-warning` token exists yet), info uses `border-info`/`info-soft`, danger uses `border-danger`/`danger-soft` — named `danger`, not `error`, to match `Button`'s own variant naming and every other danger-flavored token already in this doc. Neutral has no matching token pair either, so it composes two already-established tokens instead (`border-subtle` for the background, `text-secondary` for the bar), and is the one variant with no leading icon — every other variant leads with a Phosphor icon in that same accent color. Title and message stay on the plain `text-primary`/`text-secondary` tokens regardless of variant, so the tint never bleeds into the copy itself. `danger` renders with the assertive `role="alert"`, since it's the one variant representing something going wrong; every other variant is the polite `role="status"`. The close (×) control always renders — `onClose` is optional, so `Toast` still renders standalone (e.g. this reference page, showing all five variants at once) without real dismiss state wired up. The title itself is optional too, on request: a title-less toast shows just the message, promoted from its usual muted `text-secondary` to `text-primary` since it's now the only line rather than a supporting one underneath a title — every variant supports both states, a second axis crossed with the five color variants above rather than a variant of its own. `Toast` (`src/components/atoms/Toast`) exists as a built, tested atom with no real screen consumer yet — flagged here rather than claiming one, the same "don't build ahead of a real consumer" discipline the Icons entry above already names.
