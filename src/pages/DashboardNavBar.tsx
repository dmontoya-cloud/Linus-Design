...
export interface DashboardNavBarProps {
  ...
  title?: string
  /** When set, replaces the signed-in user info on the right with a tertiary "Exit" link to
   * this path instead — for screens (like Assessment Intro) where leaving is the one thing
   * that matters here, not who's signed in. */
  exitTo?: string
}
 ...
 * user info for a tertiary "Exit" link — both independent (see `DashboardNavBarProps`).
 */
export function DashboardNavBar({ title, exitTo }: DashboardNavBarProps = {}) {
  ...
  return (
    <header className={styles.navBar}>
      ...
      {exitTo ? (
        <Link to={exitTo} className={`${buttonClassName('tertiary', 'sm')} ${styles.exitLink}`}>
          Exit
        </Link>
      ) : (
